...
- An exotic reconciliation history (a finish that leaned on a later-reopened
  ...
  **abandoned** and counts it (`reconciliationsDowngraded`). Amounts are
  untouched.
- **Some legal books refuse to round-trip through HTML — by design.** A period
  close's sweep is regenerated by replaying `closePeriod`, so if the source
  posted entries *dated in or before a period* after that period was closed
  (legal when the date falls outside the closed range — e.g. close February,
  then post January-dated income), the regenerated closing entry would diverge
  from the recorded one. The restore detects this **before writing anything**
  and refuses with a "restore from a backup bundle instead" message. The same
  refusal covers closing entries carrying reconciliation state.
- The replay is **not one transaction** (each writer mutation commits itself).
  ...
  not an expected path — but if one happens: the error names the partial state
  (N of M entries), and the audit stream carries a **failed `ledger.restore`
  marker** (`failed: true`, with the counts) so the book itself is evidence of
  the incomplete import. Treat the target as a partial book: verify, then
  clear/re-restore. Concurrent restores are serialized in-process and refused;
  don't race two restores from different processes.
