...
export const ja: PartialMessages = {
  ...
  templates: {
    ...
    readingList: {
      ...
      guidance: 'このテンプレートはギャラリー型データベースを示します。本棚ごとに本が並び、表紙・著者・星評価つきで、同じ行のテーブルビューもあります。試してみましょう：本を評価する、別の本棚へ移す、自分の本を追加する。',
    },
    projectIntake: {
      name: 'プロジェクト受付',
    ...
    compoundGrowth: {
      ...
      guidance: 'このテンプレートはリアクティブなドキュメントを示します。月数スライダーがライブなコードブロックに値を渡し、4本の複利成長カーブを描きます。試してみましょう：スライダーをドラッグする、折りたたまれたコードを開いて計算式を変える、ページをプレゼン・エクスポートする。',
    },
    teamStatus: {
