...
export const ja: PartialMessages = {
  ...
  share: {
    ...
    linkHints: {
      ...
      localOnly:
        'このリンクはこのデバイス上でのみ機能します。送付できるリンクを取得するには、設定 → 共有と公開でこのワークスペースを公開してください。',
      browserLocal:
    ...
    effectiveDefault: {
      public: '現在のワークスペース既定では、継承したページはリンクを持つ全員に表示されます。',
      authenticated: '現在のワークスペース既定では、継承したページはサインイン済みの人のみに制限されます。',
      members: '現在のワークスペース既定では、継承したページはワークスペースのメンバーのみに制限されます。',
      restricted: '現在のワークスペース既定では、継承したページはあなたと招待した人のみに制限されます。',
    },
  ...
  forwarding: {
    partialUnscoped:
      '転送は有効ですが、アカウントがサイト用にスコープされたオーナートークンを発行しなかったため、厳格なオーディエンス分離はオフのままです（別のサイト向けに発行されたトークンは引き続き拒否され、トンネルは正常に動作します）。',
    ensureRescope:
      '転送を再開しましたが、このセッションではオーナートークンをまだサイトにスコープできませんでした。既存のアクセスは変わりません。リクエストが拒否される場合は、もう一度サインインしてください。',
