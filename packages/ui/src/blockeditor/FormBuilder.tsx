...
export const FormEditView: React.FC<{
  ...
  useLayoutEffect(() => {
    const fieldId = focusAfterRemoveRef.current;
    if (!fieldId) return;
    rowRefs.current.get(fieldId)?.focus();
            ...
            {current.fields.map((field, index) => {
              ...
              return (
                <li
                  key={field.id}
                  ref={(node) => {
                    if (node) rowRefs.current.set(field.id, node);
                    else rowRefs.current.delete(field.id);
                  }}
                  tabIndex={0}
                  aria-roledescription="Sortable form field"
                ...
                >
                  ...
                  <div className="obe-form-row-copy">
                    <span>{label}</span>
                    <code>{fieldLabel(field.kind)}</code>
                  </div>
                  ...
                  <FieldActions
                    ...
                    onMove={move}
                    onRemove={() => {
                      focusAfterRemoveRef.current = current.fields[index - 1]?.id ?? current.fields[index + 1]?.id ?? null;
                      commitFields(current.fields.filter((item) => item.id !== field.id));
                    }}
                  />
                </li>
              );
      ...
      <ConfigField label={t('formBlock.settings.databasePicker')}>
        ...
        >
          ...
          {schema.databaseId && !databases.some((page) => page.hostedDatabaseId === schema.databaseId) && (
            <option value={schema.databaseId} disabled>
              {t('formBlock.settings.unavailableDatabase', {id: schema.databaseId})}
            </option>
          )}
        </Select>
      </ConfigField>
