...
const variable = String.raw`(?:var\([^)]*\)|calc\([^;]*var\([^)]*\)[^;]*\))`;
const percentage = String.raw`-?(?:[0-9]*\.)?[0-9]+%`;
const spacingAtom = String.raw`(?:0|auto|${gridPx}|${gridRem}|${percentage}|${variable})`;
const spacingValue = `/^${spacingAtom}(?:\\s+${spacingAtom}){0,3}$/`;
const radiusAtom = String.raw`(?:0|50%|999px|9999px|4px|6px|8px|${variable})`;
...
export default {
  ...
  rules: {
    'declaration-property-value-allowed-list': {
      '/^(?:(?:padding|margin)(?:-(?:top|right|bottom|left|inline(?:-start|-end)?|block(?:-start|-end)?))?|gap|row-gap|column-gap)$/': [
        spacingValue,
      ],
