...
The server is the authoritative gate: a suggest-mode direct write is refused at the REST layer regardless of what the tool attempts, and every direct write an agent token makes is attributed to that token in the page's edit log. A page pinned to **Direct** applies remote MCP writes immediately; on a page that inherits the library default, remote MCP writes currently resolve to a suggestion unless the page itself is set to Direct.
