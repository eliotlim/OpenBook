...
# =============================================================================
# VENDORED from tauri-apps/linuxdeploy-plugin-gtk
#   commit b5eb8d05b4c0ed40107fe2158c5d8527f94568ef (2024-01-04)
# This is the exact script Tauri's bundler downloads to
# ~/.cache/tauri/linuxdeploy-plugin-gtk.sh, with ONE local change: the
# "OpenBook patch" block below. Everything else is upstream, verbatim.
#
# Why: Tauri puts externalBin sidecars in AppDir/usr/bin. OpenBook ships
# `openbook-server`, a Bun `--compile` standalone binary, on which `ldd` exits
# 1. When this plugin re-invokes linuxdeploy core (the `--appdir` step near the
# bottom), core runs `ldd` over every usr/bin entry and aborts with an uncaught
# std::runtime_error ("Failed to run ldd") -> SIGABRT (134) -- which Tauri
# surfaces as the generic `failed to run linuxdeploy`. (.deb/.rpm don't invoke
# linuxdeploy, so they build fine.) The patch moves ldd-incompatible usr/bin
# binaries aside before that scan and restores them before packaging, so
# linuxdeploy never ldd's (nor rpath-rewrites) the sidecar.
# Refs: tauri-apps/tauri#8604, linuxdeploy/linuxdeploy#147.
#
# CI seeds this into ~/.cache/tauri/ before `tauri build` (release.yml ubuntu
# leg + _appimage-diag.yml); Tauri reuses a cached plugin instead of
# downloading. To refresh: re-vendor from the pinned upstream commit and
# re-apply the single "OpenBook patch" block.
# =============================================================================
...
# --- OpenBook patch: shelter ldd-incompatible bundled binaries ---------------
# linuxdeploy core (invoked just below) runs `ldd` on every AppDir/usr/bin entry
# and SIGABRTs when ldd exits non-zero (our Bun --compile sidecar,
# openbook-server). Move those binaries out of the way first and restore them on
# exit, so they are absent during linuxdeploy's ldd scan but present in the final
# AppImage. The main app binary (a normal dynamic ELF) passes ldd and is never
# sheltered. Refs: tauri-apps/tauri#8604, linuxdeploy/linuxdeploy#147.
__ob_shelter_dir=""
...
if [ -d "$APPDIR/usr/bin" ]; then
    __ob_shelter_dir="$(mktemp -d)"
    trap __ob_restore_sheltered EXIT
    while IFS= read -r -d '' __ob_f; do
        if ! ldd "$__ob_f" >/dev/null 2>&1; then
            echo "OpenBook patch: sheltering ldd-incompatible binary '$(basename "$__ob_f")' from linuxdeploy"
            mv "$__ob_f" "$__ob_shelter_dir"/
        fi
    done < <(find "$APPDIR/usr/bin" -maxdepth 1 -type f -print0)
fi
# --- end OpenBook patch ------------------------------------------------------
