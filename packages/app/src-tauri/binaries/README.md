...
Tauri bundles the OpenBook server (`@book.dev/server`) as a sidecar executable
...
```sh
pnpm --filter @book.dev/server build:sidecar
# or for a specific target:
pnpm --filter @book.dev/server build:sidecar x86_64-pc-windows-msvc
```

This must be run before `pnpm --filter @book.dev/app tauri build`. In
