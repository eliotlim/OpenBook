...
describe('HttpDataClient database form views', () => {
  const jsonRes = (body: unknown, status = 200): Response =>
    new Response(JSON.stringify(body), {status, headers: {'content-type': 'application/json'}});

  it('wires descriptor, publish/rotate, revoke, submission, and staged uploads to encoded routes', async () => {
    const calls: Array<{url: string; init?: RequestInit}> = [];
    const result = {rowId: 'row-1', submittedAt: '2026-08-13T00:00:00.000Z'};
    const client = new HttpDataClient('https://x', undefined, {
      fetchImpl: (url, init) => {
        calls.push({url, init});
        if (String(url).endsWith('/form')) {
          return Promise.resolve(jsonRes({formConfig: {acceptingResponses: true}, fields: []}));
        }
        if (init?.method === 'DELETE') return Promise.resolve(new Response(null, {status: 204}));
        if (String(url).endsWith('/capability')) {
          return Promise.resolve(jsonRes({url: 'https://x/public#capability=secret'}, 201));
        }
        if (String(url).endsWith('/uploads')) {
          return Promise.resolve(jsonRes({token: 'staged', name: 'a.pdf', size: 3}, 201));
        }
        return Promise.resolve(jsonRes(result, 201));
      },
    });

    await expect(client.getDatabaseFormDescriptor('db/id', 'view id')).resolves.toEqual({
      formConfig: {acceptingResponses: true},
      fields: [],
    });
    await expect(client.publishDatabaseForm('db/id', 'view id')).resolves.toEqual({
      url: 'https://x/public#capability=secret',
    });
    await expect(client.revokeDatabaseForm('db/id', 'view id')).resolves.toBe(true);
    await expect(client.submitDatabaseForm('db/id', 'view id', {
      capability: 'secret',
      fields: {email: 'reader@example.com'},
      idempotencyKey: 'one',
    })).resolves.toEqual(result);
    await expect(client.uploadDatabaseFormFile('db/id', 'view id', {
      capability: 'secret',
      fieldId: 'documents',
      name: 'a.pdf',
      mime: 'application/pdf',
      bytes: new Uint8Array([0, 1, 255]),
    })).resolves.toEqual({token: 'staged', name: 'a.pdf', size: 3});

    expect(calls.map((call) => call.url)).toEqual([
      'https://x/api/databases/db%2Fid/views/view%20id/form',
      'https://x/api/databases/db%2Fid/views/view%20id/capability',
      'https://x/api/databases/db%2Fid/views/view%20id/capability',
      'https://x/api/databases/db%2Fid/views/view%20id/submissions',
      'https://x/api/databases/db%2Fid/views/view%20id/uploads',
    ]);
  ...
  it('returns null/false for hidden state and throws only validated row errors', async () => {
    const hidden = new HttpDataClient('https://x', undefined, {
      fetchImpl: () => Promise.resolve(new Response(JSON.stringify({error: 'form not found'}), {status: 404})),
    });
    await expect(hidden.getDatabaseFormDescriptor('db', 'view')).resolves.toBeNull();
    await expect(hidden.revokeDatabaseForm('db', 'view')).resolves.toBe(false);

    const rejected = new HttpDataClient('https://x', undefined, {
      fetchImpl: () => Promise.resolve(jsonRes({errors: [
        {propertyId: 'email', code: 'email_format'},
        {propertyId: 'ignored', code: 'not-real'},
      ]}, 400)),
    });
    await expect(rejected.submitDatabaseForm('db', 'view', {
      capability: 'secret',
      fields: {email: 'bad'},
      idempotencyKey: 'one',
    })).rejects.toMatchObject({
      name: 'DatabaseFormSubmissionError',
      status: 400,
      errors: [{propertyId: 'email', code: 'email_format'}],
    });
  });
});
