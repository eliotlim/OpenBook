...
export type LiveEvent =
  ...
  // Collab T0 spike: an opaque incremental Y-update (base64) relayed live for a
  // page, carrying the author's Y.Doc `clientId` so the author's own connection
  // can drop the echo. Ephemeral — never persisted (see `publishPageUpdate`).
...
export class PageHub {
  ...
  /**
   * Relay one incremental Y-update for a page to the firehose (Collab T0 spike).
   * Pure fan-out — it writes nothing to the store (the debounced snapshot save is
   * the durable checkpoint), so this is a best-effort live nudge between open
   * editors. Each firehose subscriber's `live` gate still read-filters it, and
   * the `clientId` lets a subscriber drop its own echo.
   */
